...
5. `systemd-sysupdate` installs complete A/B `/usr`, verity, signature, and UKI
   artifacts. Lowercase installed UKI names remain selectable while boot-count
   suffixes are present, and boot counting preserves a previously blessed
   image. After two update UKIs exist, a successful counted boot removes the
   installer-era Type-1 entries whose original `/usr` slot is no longer
   retained.
